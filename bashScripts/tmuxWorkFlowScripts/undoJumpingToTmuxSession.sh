...
readLastElementInUndoHistory=$(tail -n 2 "$undo_session_file" | head -n 1)
readLastElementInUndoHistoryLiterallyAndWhatUsedToBeTheCurrentSession=$(tail -n 1 "$undo_session_file")
numberOfSessionsInTheUndoList=$(wc -l < "$undo_session_file")
...
focus_terminal_window() {
        tmux detach -s "$readLastElementInUndoHistoryLiterallyAndWhatUsedToBeTheCurrentSession"
        hyprctl dispatch workspace "$terminal_window_id"
        hyprctl dispatch exec "alacritty --title 'dev' -e sh -c 'tmux attach-session -t $readLastElementInUndoHistory'"
        echo "$readLastElementInUndoHistoryLiterallyAndWhatUsedToBeTheCurrentSession" >> "$redo_session_file"
        sed -i '$d' "$undo_session_file"
        write_selected_session "$readLastElementInUndoHistory"
        notify-send "Switched to tmux session: $readLastElementInUndoHistory"
    # else
